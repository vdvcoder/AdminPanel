        ...
        <form method="POST" action="{{ route("admin.customers.update", [$customer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('banned') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="banned" id="banned" value="1" {{ $customer->banned || old('banned', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('orders') ? 'is-invalid' : '' }}" name="orders[]" id="orders" multiple>
                    @foreach($orders as $id => $order)
                        <option value="{{ $id }}" {{ (in_array($id, old('orders', [])) || $customer->orders->contains($id)) ? 'selected' : '' }}>{{ $order }}</option>
                    @endforeach
                </select>
