    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.brands.update", [$brand->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $brand->description) }}">
            ...
            <div class="form-group">
                <label for="image">{{ trans('cruds.brand.fields.image') }}</label>
...
@if(isset($brand) && $brand->image)
      var files = {!! json_encode($brand->image) !!}
          for (var i in files) {
          var file = files[i]
