    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sub-categories.update", [$subCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $subCategory->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parents') ? 'is-invalid' : '' }}" name="parents[]" id="parents" multiple>
                    @foreach($parents as $id => $parent)
                        <option value="{{ $id }}" {{ (in_array($id, old('parents', [])) || $subCategory->parents->contains($id)) ? 'selected' : '' }}>{{ $parent }}</option>
                    @endforeach
                </select>
