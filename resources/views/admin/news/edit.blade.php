...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.news.update", [$news->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="categories">{{ trans('cruds.news.fields.categorie') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple>
                    @foreach($categories as $id => $categorie)
                        <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $news->categories->contains($id)) ? 'selected' : '' }}>{{ $categorie }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.news.fields.categorie_helper') }}</span>
            </div>
            <div class="form-group">
...
@if(isset($news) && $news->image)
      var files = {!! json_encode($news->image) !!}
          for (var i in files) {
          var file = files[i]
